...
import { buildAccountMaps, toDisplayTransactions, type RawTransactionRow } from "@/lib/transactions";
...
async function getAllPayoutTransactions(): Promise<RawTransactionRow[]> {
...
export async function GET(request: NextRequest) {
  ...
  const rawTxs = await getAllPayoutTransactions();
  ...
  const disbursedMap = new Map<number, string>();
  if (txIds.length > 0) {
    const batchSize = 500;
    for (let i = 0; i < txIds.length; i += batchSize) {
      ...
      const { data: frRows } = await supabase
        .from("funds_requests")
        .select("transaction_id, admin_notes, disbursed_at")
        .in("transaction_id", batch);
      for (const fr of frRows || []) {
        if (fr.transaction_id != null) {
          if (fr.admin_notes) remarkMap.set(fr.transaction_id, fr.admin_notes as string);
          if (fr.disbursed_at) disbursedMap.set(fr.transaction_id, fr.disbursed_at as string);
        }
      }
    }
  }
  ...
  const list = transactions.map((t, idx) => {
    ...
    return {
      ...
      status: t.status,
      disbursedAt: disbursedMap.get(t.transactionId) ?? null,
      payoutRemark: remarkMap.get(t.transactionId) ?? null,
    };
  });
